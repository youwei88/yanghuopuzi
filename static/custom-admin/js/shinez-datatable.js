    ...
    var turnPage = function (obj) {
        ...
        Shinez.get(url, $(params).serialize(),function(ret){
            if (ret.status == 0) {
                ...
                $("#modal-add").find("input[type=checkbox]").bootstrapSwitch();
                $("#modal-edit").find("input[type=checkbox]").bootstrapSwitch();
                table.find("input[type=checkbox]").bootstrapSwitch();
                $(params).find(".search").attr("data-index", page.pageNum).attr("last-page", page.lastPage);
            } else if(ret.status==10200){
                sessionStorage.clear();
                location.href="/";
            }else {
                Shinez.tip("error", "数据加载失败：" + ret.info);
                $(".loading-tip").html("数据加载失败：" + ret.info).css("color", "red");
            }
